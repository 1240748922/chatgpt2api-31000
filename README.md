...
当前 `main` 默认使用镜像 `sha-4a8522e`，修复账号持续更新时快照读取失败、在取号阶段被误报为 `no_available_account` 的 503：账号数据和版本改为单条 SQL 一致读取，不要求账号池停止写入。保留上一版图片存储锁、超分/保存计时修复，以及账号换号、删除预览和可调并发功能。141 项 Python 测试及前端检查通过；云端效果需升级后验证。详情和回退注意事项见 [性能审查](./PERFORMANCE_AUDIT.md)。
之前认可的旧稳定版仍保留为 `stable-before-maintenance-sharp-20260914`，镜像为 `sha-f6a3f02`。
Compose 默认锁定 `sha-4a8522e`；如果 `.env` 没有设置 `CHATGPT2API_IMAGE_TAG`，服务器直接执行更新命令即可使用该版本。若曾手动锁定旧镜像，请修改或删除那一行，否则拉取代码不会升级应用镜像。
