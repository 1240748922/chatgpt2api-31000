...
当前 `main` 默认使用镜像 `sha-a8648f6`，包含注册代理真实前置检查和阶段诊断，以及之前版本的后台同步、上传冷却、图片超时恢复、导航与日志修复和 CPU 超分并发修复。
之前认可的旧稳定版仍保留为 `stable-before-maintenance-sharp-20260914`，镜像为 `sha-f6a3f02`。
Compose 默认锁定 `sha-a8648f6`；如果 `.env` 没有设置 `CHATGPT2API_IMAGE_TAG`，服务器直接执行更新命令即可使用该版本。
