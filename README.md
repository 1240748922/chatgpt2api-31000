...
大批量导号直接使用原来的 **账号管理 → 导入 / 添加**。AT、RT、Session/CPA/Sub2API JSON 文件导入已接入批量后台服务，无需另开页面或另填密钥，支持多文件及原有目标分组。导入窗口显示持久化进度、各阶段耗时和脱敏失败日志；关闭窗口后任务继续，重新打开可查看历史任务。后端保留独立 importer、分块读取与 SQL 批量插入；额度同步可单独勾选，批次大小通过 `.env` 的 `CHATGPT2API_IMPORT_BATCH_SIZE` 调整。完整备份恢复、OAuth 登录与远程服务器导入保留各自流程。旧 `/account-import.html` 地址会返回原导入窗口。性能实测与限制见 [导入性能记录](docs/account-import-performance.md)。
