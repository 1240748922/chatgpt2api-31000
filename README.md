...
当前 `main` 默认使用镜像 `sha-8c1da8f`，修复日志详情 HTTP 500（后处理分类遗漏响应模型）、增加删除账号前的状态/额度分页预览，并拆分超分排队与处理耗时。保留上一版上传/额度限制继续换号、跨实例导入账号可见性修复；生图及超分并发不变。具体测试和适用边界见 [性能审查](./PERFORMANCE_AUDIT.md)。
之前认可的旧稳定版仍保留为 `stable-before-maintenance-sharp-20260914`，镜像为 `sha-f6a3f02`。
Compose 默认锁定 `sha-8c1da8f`；如果 `.env` 没有设置 `CHATGPT2API_IMAGE_TAG`，服务器直接执行更新命令即可使用该版本。若曾手动锁定旧镜像，请修改或删除那一行，否则拉取代码不会升级应用镜像。
