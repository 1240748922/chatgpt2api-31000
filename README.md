...
当前 `main` 默认使用镜像 `sha-d8b7cdc`，修复新图保存被全量图片索引锁和无关图片锁阻塞的问题；补齐每次尝试的超分/保存计时，存储失败不再换号重复生图。保留账号换号、删除预览和可调并发功能。124 项 Python 测试及前端检查通过；云端实际延迟需升级后验证。详情和回退注意事项见 [性能审查](./PERFORMANCE_AUDIT.md)。
之前认可的旧稳定版仍保留为 `stable-before-maintenance-sharp-20260914`，镜像为 `sha-f6a3f02`。
Compose 默认锁定 `sha-d8b7cdc`；如果 `.env` 没有设置 `CHATGPT2API_IMAGE_TAG`，服务器直接执行更新命令即可使用该版本。若曾手动锁定旧镜像，请修改或删除那一行，否则拉取代码不会升级应用镜像。
