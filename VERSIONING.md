...
| 新方案修复版（main 默认） | `sha-26d613e` | 加入按生图负载延后维护、低库存放行补号，并包含 Sharp 依赖修复 |

先前创建的 `stable-20260913` 标签误把新方案当成稳定版，已弃用；不要用它回退旧方案。为避免已拉取标签的电脑和服务器产生歧义，不重写该标签，改用上面的新稳定标签。
`main` 保留新方案代码，Compose 默认运行 `sha-26d613e`。直接用源码启动或自行构建 `main` 也会运行新方案；需要旧方案源码时使用 `stable-before-maintenance-sharp-20260914`。
...
```env
CHATGPT2API_IMAGE_TAG=sha-26d613e
```
...
配置输出中的应用镜像应以 `:sha-26d613e` 结尾。如果仍然是 `latest`，检查部署文件是否更新，或者终端是否设置了覆盖 `.env` 的同名环境变量。可用 `unset CHATGPT2API_IMAGE_TAG` 清除终端覆盖后重试。
...
```bash
docker image save -o "$RELEASE_BACKUP/stable-image.tar" ghcr.io/1240748922/chatgpt2api-31000:sha-26d613e
```
...
当前新维护方案的修复镜像是 `sha-26d613e`。主动使用该方案时，把 `.env` 改成 `CHATGPT2API_IMAGE_TAG=sha-26d613e`；如果需要恢复旧方案，改回 `sha-f6a3f02`。
