...
`main` 保留新方案代码供继续测试，但 Compose 默认运行旧稳定镜像。直接用源码启动或自行构建 `main` 会运行新方案；需要旧方案源码时使用 `stable-before-maintenance-sharp-20260914`。
...
配置输出中的应用镜像应以 `:sha-f6a3f02` 结尾。如果仍然是 `latest`，检查部署文件是否更新，或者终端是否设置了覆盖 `.env` 的同名环境变量。可用 `unset CHATGPT2API_IMAGE_TAG` 清除终端覆盖后重试。
...
```bash
docker image save -o "$RELEASE_BACKUP/stable-image.tar" ghcr.io/1240748922/chatgpt2api-31000:sha-f6a3f02
```
...
本次新维护方案的测试镜像是 `sha-f1648e6`，也可通过 Git 标签 `preview-maintenance-20260913` 查看其应用源码。主动测试该方案时，把 `.env` 改成 `CHATGPT2API_IMAGE_TAG=sha-f1648e6`；测试后改回 `sha-f6a3f02` 即可恢复旧方案。
