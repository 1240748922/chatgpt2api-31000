...
| 当前修复版（main 默认） | `sha-2137f15` | 额度耗尽自动换号、超分/后处理耗时记录、账号清理操作、系统设置可调每实例 1–512 个超分并发；保留生图 SSE、图片任务输出和初始化连接重试修复 |
...
`main` 保留新方案代码，Compose 默认运行 `sha-2137f15`。直接用源码启动或自行构建 `main` 也会运行新方案；需要旧方案源码时使用 `stable-before-maintenance-sharp-20260914`。
...
当前镜像 `sha-2137f15` 会由本次提交触发 GitHub Actions 构建；请先等待 Actions 显示成功再拉取。如果 `.env` 锁定旧标签，需要改为这个标签，或删除 `CHATGPT2API_IMAGE_TAG` 行后跟随仓库默认值，再拉取并重建。
