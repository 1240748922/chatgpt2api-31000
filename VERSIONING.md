...
| 项目 | 固定版本 |
| --- | --- |
| 稳定版 Git 标签（含版本锁定配置和本文） | `stable-20260913` |
| 应用代码提交 | `f1648e6407c47b7101b32847337a9ee195f253b0` |
| 应用镜像 | `ghcr.io/1240748922/chatgpt2api-31000:sha-f1648e6` |

这是用户当前认可的稳定基线。稳定标签加入的变更仅涉及部署版本选择和文档，应用代码保持不变。
该应用提交的 [GitHub Actions 构建](https://github.com/1240748922/chatgpt2api-31000/actions/runs/34743871652) 已成功。
...
配置输出中的应用镜像应以 `:sha-f1648e6` 结尾。如果仍然是 `latest`，检查部署文件是否更新，或者终端是否设置了覆盖 `.env` 的同名环境变量。可用 `unset CHATGPT2API_IMAGE_TAG` 清除终端覆盖后重试。
...
```bash
docker image save -o "$RELEASE_BACKUP/stable-image.tar" ghcr.io/1240748922/chatgpt2api-31000:sha-f1648e6
```
...
git switch --detach stable-20260913
```

此时 `.env` 等被 Git 忽略的实际配置仍然保留。把 `.env` 的镜像值改回 `sha-f1648e6`，然后按第 1 节重新部署。处于稳定标签时不要执行 `git pull`；以后要继续升级，先 `git switch main`，再按第 3 节操作。
