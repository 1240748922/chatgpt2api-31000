...
| 当前修复版（main 默认） | `sha-a831bd9` | 修复生图 SSE 提前关闭、接收任务接口的图片输出、初始化连接有界重试；保留之前的维护、超分与上传重试修复 |
...
`main` 保留新方案代码，Compose 默认运行 `sha-a831bd9`。直接用源码启动或自行构建 `main` 也会运行新方案；需要旧方案源码时使用 `stable-before-maintenance-sharp-20260914`。
...
当前镜像 `sha-a831bd9` 的 [GitHub Actions 构建](https://github.com/1240748922/chatgpt2api-31000/actions/runs/35003286219) 已成功，可以拉取。如果 `.env` 锁定旧标签，需要改为这个标签，或删除 `CHATGPT2API_IMAGE_TAG` 行后跟随仓库默认值，再拉取并重建。
