...
| 项目 | 固定版本 |
| --- | --- |
| 稳定版 Git 标签（含版本锁定配置和本文） | `stable-before-maintenance-20260913` |
| 应用代码提交 | `16e719362c207d7fc28a19ff9450178942d8cd95` |
| 应用镜像 | `ghcr.io/1240748922/chatgpt2api-31000:sha-16e7193` |
...
| 版本用途 | 应用镜像 | 说明 |
| --- | --- | --- |
| 稳定版（默认） | `sha-16e7193` | 新维护方案之前的版本 |
| 新方案测试版 | `sha-f1648e6` | 加入按生图负载延后维护、低库存放行补号的逻辑，需服务器验证 |

先前创建的 `stable-20260913` 标签误把新方案当成稳定版，已弃用；不要用它回退旧方案。为避免已拉取标签的电脑和服务器产生歧义，不重写该标签，改用上面的新稳定标签。
`main` 保留新方案代码供继续测试，但 Compose 默认运行旧稳定镜像。直接用源码启动或自行构建 `main` 会运行新方案；需要旧方案源码时使用 `stable-before-maintenance-20260913`。
如果之前已经在服务器 `.env` 中写了 `CHATGPT2API_IMAGE_TAG=sha-f1648e6`，本次 `git pull` 不会覆盖它，必须手动改为 `sha-16e7193` 才会回到旧方案。
...
配置输出中的应用镜像应以 `:sha-16e7193` 结尾。如果仍然是 `latest`，检查部署文件是否更新，或者终端是否设置了覆盖 `.env` 的同名环境变量。可用 `unset CHATGPT2API_IMAGE_TAG` 清除终端覆盖后重试。
Compose 未配置这项时也默认使用该稳定镜像，但建议写入 `.env`，使今后的 Git 更新继续保留你的选择。
...
```bash
docker image save -o "$RELEASE_BACKUP/stable-image.tar" ghcr.io/1240748922/chatgpt2api-31000:sha-16e7193
```
...
本次新维护方案的测试镜像是 `sha-f1648e6`，也可通过 Git 标签 `preview-maintenance-20260913` 查看其应用源码。主动测试该方案时，把 `.env` 改成 `CHATGPT2API_IMAGE_TAG=sha-f1648e6`；测试后改回 `sha-16e7193` 即可恢复旧方案。
...
git switch --detach stable-before-maintenance-20260913
```

此时 `.env` 等被 Git 忽略的实际配置仍然保留。把 `.env` 的镜像值改回 `sha-16e7193`，然后按第 1 节重新部署。处于稳定标签时不要执行 `git pull`；以后要继续升级，先 `git switch main`，再按第 3 节操作。
