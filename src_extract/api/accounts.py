...
class RegistrationProviderConfigPatch(BaseModel):
    registration_proxy: str | None = None
    registration_proxy_pool: str | None = None
    remail_base_url: str | None = None
    remail_api_key: str | None = None
    smailr_base_url: str | None = None
    smailr_api_key: str | None = None
    cfworker_url: str | None = None
    cfworker_admin_token: str | None = None
    cfworker_api_token: str | None = None
...
def create_router() -> APIRouter:
    ...
    from services.account_maintenance_status import maintenance_view
    ...
    async def get_maintenance_status(authorization: str | None = Header(default=None)):
        ...
        return await run_in_threadpool(maintenance_view)
    ...
    async def get_account_availability(authorization: str | None = Header(default=None)):
        ...
        return {**result, **(await run_in_threadpool(maintenance_view)), "prewarm": page_prewarm_pool.status()}
    ...
    @router.get("/api/accounts/replenishment")
    async def get_account_replenishment_status(authorization: str | None = Header(default=None)):
        require_admin(authorization)
        return await run_in_threadpool(account_replenishment_service.status)

    @router.get("/api/accounts/replenishment/provider-config")
    ...
    async def update_account_replenishment_provider_config(
        body: RegistrationProviderConfigPatch,
        authorization: str | None = Header(default=None),
    ):
        require_admin(authorization)
        try:
            return await run_in_threadpool(
                account_replenishment_service.update_provider_config,
                body.model_dump(exclude_none=True),
            )
        except (FileNotFoundError, ValueError) as exc:
            raise HTTPException(status_code=400, detail={"error": str(exc)}) from exc

    @router.post("/api/accounts/replenishment/mailbox-file")
    async def import_account_replenishment_mailbox_file(
        body: MailboxFileImportRequest,
        authorization: str | None = Header(default=None),
    ):
        require_admin(authorization)
        try:
            return await run_in_threadpool(
                account_replenishment_service.import_mailbox_file,
                body.filename,
                body.content,
            )
        except (FileNotFoundError, ValueError) as exc:
    ...
    @router.post("/api/accounts/replenishment/run")
    async def run_account_replenishment(
        force: bool = False,
        manual_count: int | None = Query(default=None, ge=1, le=1000),
        authorization: str | None = Header(default=None),
    ):
        require_admin(authorization)
        if manual_count is not None:
            return await run_in_threadpool(
                account_replenishment_service.start_manual,
                manual_count,
            )
        return await run_in_threadpool(
            account_replenishment_service.run_once,
            force=force or manual_count is not None,
            requested_count=manual_count,
        )
