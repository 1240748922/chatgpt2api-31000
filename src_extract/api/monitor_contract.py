...
class MonitorAccountProcessingView(StrictMonitorModel):
    limit: int = Field(ge=0)
    active: int = Field(ge=0)
    waiting: int = Field(ge=0)


class MonitorCapacityView(StrictMonitorModel):
    limit: int = Field(ge=0)
    active: int = Field(ge=0)
    waiting: int = Field(ge=0)
...
class MonitorAccountShardView(StrictMonitorModel):
...
class MonitorThreadpoolView(StrictMonitorModel):
    ...
    runtime_threads: int = Field(ge=0)
    account_processing: MonitorAccountProcessingView
    image: MonitorCapacityView
    upscale: MonitorCapacityView
    account_shard: MonitorAccountShardView
...
class MonitorEventView(StrictMonitorModel):
    ...
    account_token_writer_lock_ms: int | None = None
    account_token_dispatch_lock_ms: int | None = None
    ...
    upload_decode_ms: int | None = None
