...
def _account_cleanup_payload(body: AccountCleanupRequest | None = None, *, dry_run: bool) -> dict[str, Any]:
    ...
    if dry_run:
        return account_service.preview_auto_remove_accounts(
            **kwargs, limit=body.preview_limit, offset=body.preview_offset,
        )
...
def create_router(app_version: str) -> APIRouter:
    ...
    @router.get("/internal/monitor/operations")
    async def get_internal_operations(x_cluster_monitor_secret: str | None = Header(default=None)):
        try:
            # O(1) memory counter only; don't queue behind image/maintenance IO.
            return local_internal_operations(x_cluster_monitor_secret or "")
    ...
    @router.get("/internal/monitor/realtime/{call_id}")
    async def get_internal_realtime_monitor_call(
        call_id: str,
        x_cluster_monitor_secret: str | None = Header(default=None),
    ):
        try:
            record = local_internal_call_detail(call_id, x_cluster_monitor_secret or "")
        except PermissionError as exc:
            raise HTTPException(status_code=403, detail={"error": "forbidden"}) from exc
        if record is None:
    ...
    async def get_realtime_monitor(authorization: str | None = Header(default=None)):
        require_admin(authorization)
        return await run_in_threadpool(lambda: build_monitor_view(cluster_monitor_snapshot()))
    ...
    async def get_realtime_monitor_call(
        ...
        require_admin(authorization)
        record = await run_in_threadpool(cluster_monitor_call_detail, call_id)
        if record is None:
            raise HTTPException(status_code=404, detail={"error": "monitor call not found"})
