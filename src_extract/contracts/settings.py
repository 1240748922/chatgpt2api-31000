...
class _AccountReplenishmentFields(_StrictModel):
    enabled: bool = False
    registration_source: str = "configured"
    registration_mode: Literal["password", "passwordless", "har", "legacy"] = "password"
    minimum_available: int = _numeric_field("account_replenishment.minimum_available")
    target_available: int = _numeric_field("account_replenishment.target_available")
    interval_seconds: int = _numeric_field("account_replenishment.interval_seconds")
    cooldown_seconds: int = _numeric_field("account_replenishment.cooldown_seconds")
    max_batch_size: int = _numeric_field("account_replenishment.max_batch_size")
    workers: int = _numeric_field("account_replenishment.workers")
    launch_timeout_seconds: int = _numeric_field("account_replenishment.launch_timeout_seconds")
    workdir: str = ""
    output_dir: str = ""
    entrypoint: str = "chatgpt_phone_reg.py"
    python_executable: str = ""
    extra_args: str = ""
    mailbox_file: str = ""
    remail_service_mode: str = "purchase"
    remail_supply: str = "private_first"
    remail_email_suffix: str = "outlook.com"
    remail_project_id: int = 2
    cfworker_domain: str = ""
    smailr_domain: str = ""
    import_new_sessions: bool = True
    no_2fa: bool = True
    no_phone_reuse: bool = True


class AccountReplenishmentSettings(_AccountReplenishmentFields):
    pass


class AccountReplenishmentPatch(_AccountReplenishmentFields):
    pass
...
class _SettingsEditableFields(_StrictModel):
    ...
    account_replenishment: AccountReplenishmentPatch = Field(default_factory=AccountReplenishmentPatch)
...
class SettingsValues(_SettingsEditableFields):
    ...
    account_replenishment: AccountReplenishmentSettings = Field(default_factory=AccountReplenishmentSettings)
