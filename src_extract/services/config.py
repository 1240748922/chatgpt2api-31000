...
def _normalize_account_replenishment_settings(value: object) -> dict[str, object]:
    source = value if isinstance(value, dict) else {}
    normalized = copy.deepcopy(source)
    normalized.update({
        "enabled": _normalize_bool(source.get("enabled"), False),
        "registration_source": str(source.get("registration_source") or "configured").strip().lower() or "configured",
        "registration_mode": (
            str(source.get("registration_mode") or "password").strip().lower()
            if str(source.get("registration_mode") or "password").strip().lower()
            in {"password", "passwordless", "har", "legacy"}
            else "password"
        ),
        "minimum_available": _normalize_positive_int(source.get("minimum_available"), 0, 0),
        "target_available": _normalize_positive_int(source.get("target_available"), 0, 0),
        "interval_seconds": _normalize_positive_int(source.get("interval_seconds"), 60, 5),
        "cooldown_seconds": _normalize_positive_int(source.get("cooldown_seconds"), 120, 0),
        "max_batch_size": _normalize_positive_int(source.get("max_batch_size"), 50, 1),
        "workers": _normalize_positive_int(source.get("workers"), 4, 1),
        "launch_timeout_seconds": _normalize_positive_int(
            source.get("launch_timeout_seconds"),
            21600,
            300,
        ),
        "workdir": str(source.get("workdir") or "").strip(),
        "output_dir": str(source.get("output_dir") or "").strip(),
        "entrypoint": str(source.get("entrypoint") or "chatgpt_phone_reg.py").strip() or "chatgpt_phone_reg.py",
        "python_executable": str(source.get("python_executable") or "").strip(),
        "extra_args": str(source.get("extra_args") or "").strip(),
        "mailbox_file": str(source.get("mailbox_file") or "").strip(),
        "remail_service_mode": str(source.get("remail_service_mode") or "purchase").strip().lower() or "purchase",
        "remail_supply": str(source.get("remail_supply") or "private_first").strip().lower() or "private_first",
        "remail_email_suffix": str(source.get("remail_email_suffix") or "outlook.com").strip() or "outlook.com",
        "remail_project_id": _normalize_positive_int(source.get("remail_project_id"), 2, 1),
        "cfworker_domain": str(source.get("cfworker_domain") or "").strip(),
        "smailr_domain": str(source.get("smailr_domain") or "").strip(),
        "import_new_sessions": _normalize_bool(source.get("import_new_sessions"), True),
        "no_2fa": _normalize_bool(source.get("no_2fa"), True),
        "no_phone_reuse": _normalize_bool(source.get("no_phone_reuse"), True),
    })
    return normalized
...
class ConfigStore:
    ...
    @property
    def account_replenishment(self) -> dict[str, object]:
        self.reload_if_changed()
        return _normalize_account_replenishment_settings(self.data.get("account_replenishment"))
    ...
    def get(self) -> dict[str, object]:
        with self._lock:
            ...
            data["account_replenishment"] = self.account_replenishment
