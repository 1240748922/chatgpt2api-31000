...
def _build_diagnostic_groups(
    ...
    threadpool: Mapping[str, Any],
    completed_window_text: str,
) -> list[dict[str, Any]]:
    thread_tokens = _int(threadpool.get("tokens"))
    runtime_threads = _int(threadpool.get("runtime_threads"))
    account_processing = _mapping(threadpool.get("account_processing"))
    account_processing_limit = _int(account_processing.get("limit"))
    account_processing_active = _int(account_processing.get("active"))
    account_processing_waiting = _int(account_processing.get("waiting"))
    image_pool = _mapping(threadpool.get("image"))
    upscale_pool = _mapping(threadpool.get("upscale"))
    ...
    return [
        ...
        },
        {
            ...
            "meta": "本地线程、请求入口、代理出口",
            "items": [
                _diagnostic_item("thread_capacity", "通用线程容量", thread_tokens, "文本 / 管理接口"),
                _diagnostic_item("image_capacity", "图片线程容量", _int(image_pool.get("limit")), f"活跃 {_int(image_pool.get('active'))} · 排队 {_int(image_pool.get('waiting'))}", "warning" if _int(image_pool.get("waiting")) else "info"),
                _diagnostic_item("upscale_capacity", "图片放大容量", _int(upscale_pool.get("limit")), f"活跃 {_int(upscale_pool.get('active'))} · 排队 {_int(upscale_pool.get('waiting'))}", "warning" if _int(upscale_pool.get("waiting")) else "info"),
                _diagnostic_item("runtime_threads", "进程实际线程", runtime_threads, "Python 活跃线程"),
                _diagnostic_item("account_processing_active", "账号维护活跃", account_processing_active, f"上限 {account_processing_limit}", "info"),
                _diagnostic_item("account_processing_waiting", "账号维护排队", account_processing_waiting, f"上限 {account_processing_limit}", "warning" if account_processing_waiting else "muted"),
...
def build_monitor_view(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    account_processing_source = _mapping(threadpool_source.get("account_processing"))
    image_source = _mapping(threadpool_source.get("image"))
    upscale_source = _mapping(threadpool_source.get("upscale"))
    account_shard_source = _mapping(threadpool_source.get("account_shard"))
    threadpool = {
        "tokens": _int(threadpool_source.get("tokens")),
        "previous_tokens": _int(threadpool_source.get("previous_tokens")),
        "runtime_threads": _int(threadpool_source.get("runtime_threads")),
        "account_processing": {
            "limit": _int(account_processing_source.get("limit")),
            "active": _int(account_processing_source.get("active")),
            "waiting": _int(account_processing_source.get("waiting")),
        },
        "image": {"limit": _int(image_source.get("limit")), "active": _int(image_source.get("active")), "waiting": _int(image_source.get("waiting"))},
        "upscale": {"limit": _int(upscale_source.get("limit")), "active": _int(upscale_source.get("active")), "waiting": _int(upscale_source.get("waiting"))},
        "account_shard": {
            "count": max(1, _int(account_shard_source.get("count"))),
            "index": _int(account_shard_source.get("index")),
            "assigned_accounts": _int(account_shard_source.get("assigned_accounts")),
            "ready_accounts": _int(account_shard_source.get("ready_accounts")),
            "inflight": _int(account_shard_source.get("inflight")),
        },
    }
    window = {
    ...
    return {
        ...
        "diagnostic_groups": _build_diagnostic_groups(summary, threadpool, completed_window_text),
    }
