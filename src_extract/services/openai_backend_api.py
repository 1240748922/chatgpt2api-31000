...
from utils.helper import UpstreamHTTPError, ensure_ok, iter_sse_payloads, new_uuid, split_image_model, parse_retry_after
...
# Account maintenance is outside the image request hot path. Keep its three
# independent upstream lookups parallel, but bound their process-wide thread
# footprint instead of creating three new threads for every account check.
_USER_INFO_EXECUTOR = ThreadPoolExecutor(
    max_workers=12,
    thread_name_prefix="account-user-info",
)
...
class OpenAIBackendAPI:
    ...
    def _image_request_timeout(
            self,
            maximum: float,
            message: str = "image request deadline exceeded",
    ) -> float:
        deadline = self.deadline_monotonic
        if deadline is None:
            return max(0.001, float(maximum))
        remaining = float(deadline) - time.monotonic()
        if remaining <= 0:
            raise ImageFailureError(
                message,
                failure=image_failure("task_interrupted", raw_detail=message),
            )
        return min(max(0.001, float(maximum)), max(0.001, remaining))
    ...
    def get_user_info(self) -> Dict[str, Any]:
        ...
        futures = (
            _USER_INFO_EXECUTOR.submit(self._get_me),
            _USER_INFO_EXECUTOR.submit(self._get_conversation_init),
            _USER_INFO_EXECUTOR.submit(self._get_default_account),
        )
        results = []
        errors = []
    ...
    def iter_codex_image_response_events(
        ...
        stream_timeout = self._image_request_timeout(config.image_stream_timeout_secs)
        try:
            ...
            try:
                try:
                    conversation = self._get_conversation(
                        conversation_id, timeout_secs=max(0.001, min(10.0, _remaining()))
                    )
                finally:
            ...
            if (
                ...
                try:
                    tasks = self._query_backend_tasks(
                        conversation_id=conversation_id,
                        timeout_secs=max(0.001, min(3.0, _remaining())),
                    )
    ...
    def resolve_conversation_image_urls(
        ...
        timeout = self._image_request_timeout(timeout)
