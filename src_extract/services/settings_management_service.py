...
from contracts.settings import (
    AIReviewSettings,
    AccountReplenishmentPatch,
    AccountReplenishmentSettings,
...
_FIELD_SPECS: dict[str, dict[str, Any]] = {
    ...
    "account_replenishment.enabled": _field_metadata(False),
    "account_replenishment.registration_source": _field_metadata("configured", options=("configured", "remail_target", "mailbox_file", "cfworker", "smailr", "phone")),
    "account_replenishment.registration_mode": _field_metadata("password", options=("password", "passwordless", "har", "legacy")),
    "account_replenishment.minimum_available": _numeric_field_metadata("account_replenishment.minimum_available"),
    "account_replenishment.target_available": _numeric_field_metadata("account_replenishment.target_available"),
    "account_replenishment.interval_seconds": _numeric_field_metadata("account_replenishment.interval_seconds"),
    "account_replenishment.cooldown_seconds": _numeric_field_metadata("account_replenishment.cooldown_seconds"),
    "account_replenishment.max_batch_size": _numeric_field_metadata("account_replenishment.max_batch_size"),
    "account_replenishment.workers": _numeric_field_metadata("account_replenishment.workers"),
    "account_replenishment.launch_timeout_seconds": _numeric_field_metadata("account_replenishment.launch_timeout_seconds"),
    "account_replenishment.workdir": _field_metadata(""),
    "account_replenishment.output_dir": _field_metadata(""),
    "account_replenishment.entrypoint": _field_metadata("chatgpt_phone_reg.py"),
    "account_replenishment.python_executable": _field_metadata(""),
    "account_replenishment.extra_args": _field_metadata(""),
    "account_replenishment.mailbox_file": _field_metadata(""),
    "account_replenishment.remail_service_mode": _field_metadata("purchase", options=("purchase", "code")),
    "account_replenishment.remail_supply": _field_metadata("private_first", options=("private_first", "public_only")),
    "account_replenishment.remail_email_suffix": _field_metadata("outlook.com"),
    "account_replenishment.remail_project_id": _field_metadata(2, min=1),
    "account_replenishment.cfworker_domain": _field_metadata(""),
    "account_replenishment.smailr_domain": _field_metadata(""),
    "account_replenishment.import_new_sessions": _field_metadata(True),
    "account_replenishment.no_2fa": _field_metadata(True),
    "account_replenishment.no_phone_reuse": _field_metadata(True),
...
class SettingsManagementService:
    ...
    def _project_settings(self, effective: dict[str, Any], stored: dict[str, Any]) -> SettingsValues:
        ...
        account_replenishment = _dict(effective.get("account_replenishment"))
        ...
        return SettingsValues(
            ...
            account_replenishment=AccountReplenishmentSettings(
                enabled=_bool(account_replenishment.get("enabled"), False),
                registration_source=_text(account_replenishment.get("registration_source"), "configured") or "configured",
                registration_mode=_enum(
                    account_replenishment.get("registration_mode"),
                    "password",
                    {"password", "passwordless", "har", "legacy"},
                ),
                minimum_available=normalize_integer_setting(
                    "account_replenishment.minimum_available",
                    account_replenishment.get("minimum_available"),
                ),
                target_available=normalize_integer_setting(
                    "account_replenishment.target_available",
                    account_replenishment.get("target_available"),
                ),
                interval_seconds=normalize_integer_setting(
                    "account_replenishment.interval_seconds",
                    account_replenishment.get("interval_seconds"),
                ),
                cooldown_seconds=normalize_integer_setting(
                    "account_replenishment.cooldown_seconds",
                    account_replenishment.get("cooldown_seconds"),
                ),
                max_batch_size=normalize_integer_setting(
                    "account_replenishment.max_batch_size",
                    account_replenishment.get("max_batch_size"),
                ),
                workers=normalize_integer_setting(
                    "account_replenishment.workers",
                    account_replenishment.get("workers"),
                ),
                launch_timeout_seconds=normalize_integer_setting(
                    "account_replenishment.launch_timeout_seconds",
                    account_replenishment.get("launch_timeout_seconds"),
                ),
                workdir=_text(account_replenishment.get("workdir")),
                output_dir=_text(account_replenishment.get("output_dir")),
                entrypoint=_text(account_replenishment.get("entrypoint"), "chatgpt_phone_reg.py") or "chatgpt_phone_reg.py",
                python_executable=_text(account_replenishment.get("python_executable")),
                extra_args=_text(account_replenishment.get("extra_args")),
                mailbox_file=_text(account_replenishment.get("mailbox_file")),
                remail_service_mode=_text(account_replenishment.get("remail_service_mode"), "purchase") or "purchase",
                remail_supply=_text(account_replenishment.get("remail_supply"), "private_first") or "private_first",
                remail_email_suffix=_text(account_replenishment.get("remail_email_suffix"), "outlook.com") or "outlook.com",
                remail_project_id=max(1, int(account_replenishment.get("remail_project_id") or 2)),
                cfworker_domain=_text(account_replenishment.get("cfworker_domain")),
                smailr_domain=_text(account_replenishment.get("smailr_domain")),
                import_new_sessions=_bool(account_replenishment.get("import_new_sessions"), True),
                no_2fa=_bool(account_replenishment.get("no_2fa"), True),
                no_phone_reuse=_bool(account_replenishment.get("no_phone_reuse"), True),
            ),
