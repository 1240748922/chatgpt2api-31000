...
class AccountReplenishmentStateModel(DatabaseBase):
    __tablename__ = "account_replenishment_state"
...
class AccountReplenishmentRepository(_JsonStateRepository):
    def __init__(self, database_url: str | None = None) -> None:
        super().__init__(AccountReplenishmentStateModel, "account_replenishment_state", database_url)

    def run_lock(self, *, timeout_seconds: float | None = None) -> _DatabaseLease:
        return _DatabaseLease(
            self,
            "account_replenishment:run",
            lease_seconds=300,
            timeout_seconds=timeout_seconds,
        )
    ...
    def scheduler_leader_lock(self, *, timeout_seconds: float = 0) -> _DatabaseLease:
        return _DatabaseLease(
            self,
            "account_replenishment:scheduler_leader",
            lease_seconds=5,
            timeout_seconds=timeout_seconds,
        )
